...
//    private long msgId;
        ...
        public void handleMessage(Message msg) {
            ...
            case HTTP_OVER:
                ...
                if (null == s || !s.contains("|")) {
                    Toast.makeText(AddRemindActivity.this, "网络连接失败，请确认后重试.", Toast.LENGTH_SHORT).show();
                }
                ...
                if (!ss[0].equals("200")) {
                    ...
                    Toast.makeText(AddRemindActivity.this, "失败，请重试", Toast.LENGTH_SHORT).show();
                } else {
                    // 成功
                    ...
                    try {
                        ...
                        // 设置事件驱动的id
                        messageEntity.setRemindId(jsonObject.getString("id"));
            ...
            case NOTIFY_SUCCESS:
                ...
                if (isForSelf) {
                    // TODO 测试为自己添加任务,
                    // addSelf()需要去掉，isForSelf需要重新取值，将当前currentPeopel.getNum()与AppUtil.getPhoneNumber(this)比较，相同则为true
     ...
     * 监听重复周期改变
     */
    // private OnCheckedChangeListener repeatCheckListener = new
    ...
    public void onClick(View v) {
        ...
        case R.id.select_peopel_btn:
            // 选择联系人
            // Intent intentP = new Intent(AddRemindActivity.this,
        ...
        case R.id.select_type_btn:
            ...
            // @Override
            // public void result(boolean isComplete, String result, String
            // error) {
            // // do something...
            // //
            // System.out.println("isComplete:"+isComplete+";result:"+result+";error:"+error);
            // Log.e("Upload_CompleteListener",
            // "isComplete:"+isComplete+";result:"+result+";error:"+error);
            // }
            // },
            ...
            // totalBytes);
            // Log.e("Upload_ProgressListener", "trans:" + transferedBytes +
            // "; total:" + totalBytes);
            // }
            // });
            // }
    ...
    private void createNotify() {
        String remindTime = selectDateBtn.getText().toString() + " " + selectTimeBtn.getText().toString();

        String remindTimeMili = System.currentTimeMillis() + "";
        remindEntity = new RemindEntity("", AppConstant.USER_NUM, targetPeopel.getNum(), targetPeopel.getName(),
                targetPeopel.getNickName(), AppUtil.getNowTime(), remindTimeMili, "一起去吃饭吧", remindTime, remindTime,
                selectTitleBtn.getText().toString(), repeatType, isPreview, "", "", RemindEntity.READ);
        ...
        // 加入到message数据库
        messageEntity = new MessageEntity("", targetPeopel.getName(), targetPeopel.getNum(), user.getNickName(),
                AppConstant.USER_NUM, AppUtil.getNowTime(), MessageEntity.SENDING, MessageEntity.NORMAL,
                MessageEntity.TYPE_REMIND, "", "", targetPeopel.getNum(), MessageEntity.TYPE_SEND,
                remindEntity.getContent(), MessageEntity.FEED_DEFAULT, AppConstant.USER_NUM, "");

        if (isForSelf) {
            // TODO 测试为自己添加任务,
            // addSelf()需要去掉，isForSelf需要重新取值，将当前currentPeopel.getNum()与AppUtil.getPhoneNumber(this)比较，相同则为true
            // AppUtil.setAlarm(this, remindEntity.getRemindTime(), (int)
            // remindId);
            ...
            if (NetWorkUtil.isAvailable(AddRemindActivity.this)) {
                ...
                new Thread(new Runnable() {
                    ...
                    @Override
                    public void run() {
                        String params = HttpClient.getCreateNofiJsonForPost(targetPeopel.getFriendId(), AppConstant.FROM_ID,
                                remindEntity.getTitle(), remindEntity.getIsPreview() + "", remindEntity.getRemindTime(),
                                remindEntity.getRepeatType(), AppConstant.USER_NUM, remindEntity.getNickName(),
                                remindEntity.getContent(), remindEntity.getAddTime(), remindEntity.getRemindState(),
                                remindEntity.getRemindMethod(), remindEntity.getAudioPath(), remindEntity.getImgPath(),
                                remindEntity.getVideoPath());
                        String s = HttpClient.post(HttpClient.url + HttpClient.create_notify, params);
