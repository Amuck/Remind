...
public class LoginActivity extends LoginBaseActivity implements OnClickListener {
     ...
     * 用账号输入
     */
    private ClearEditText et_user_account;
    /**
     * 用户密码输入
     */
     ...
     * 用户昵称
     */
    private ClearEditText et_user_nick;
    /**
     * 是否记住用户名
     */
    ...
    private void init() {
        ...
        et_user_account = (ClearEditText) findViewById(R.id.et_user_name);
        et_user_nick = (ClearEditText) findViewById(R.id.et_user_nick);
        ...
        et_user_account.setOnTouchListener(new OnTouchListener() {
        ...
        et_user_nick.setOnTouchListener(new OnTouchListener() {
        ...
        login_user_reg.setOnCheckedChangeListener(new OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                ...
                if (isChecked) {
                    // 注册
                    isRegist = true;
                    titleStr = getResources().getString(R.string.user_login_title);
                    btnStr = getResources().getString(R.string.user_reg);
                    visible = View.INVISIBLE;
                    et_user_nick.setVisibility(View.VISIBLE);
                } else {
                    // 登陆
                    isRegist = false;
                    titleStr = getResources().getString(R.string.user_reg_title);
                    btnStr = getResources().getString(R.string.user_login);
                    visible = View.VISIBLE;
                    et_user_nick.setVisibility(View.GONE);
                }
        ...
        String name = MySharedPreferencesLoginType.getString(getApplicationContext(), MySharedPreferencesLoginType.USERNAME);
        if (!TextUtils.isEmpty(name)) {
            login_remember.setChecked(true);
            et_user_account.setText(name);
        }
    }
    ...
    public void onClick(View v) {
        ...
        case R.id.btn_login:
            // 登陆/注册
            mobile = et_user_account.getEditableText().toString();
            pwdStr = et_user_psw.getEditableText().toString();
            nickStr = et_user_nick.getEditableText().toString();
            if (mobile.length() == 0) {
                // 帐号不能为空
            ...
            } else if (nickStr.length() == 0 && isRegist) {
                // 您的昵称不能为空
                tv_tips.setText(getResources().getString(R.string.nick_null));
            } else if (pwdStr.length() < 6) {
                // 您的密码不能少于六位
                tv_tips.setText(getResources().getString(R.string.pwd_len_error));
            } else {
                String params = "";
                if (!isRegist) {
                    // 登陆
                    params = HttpClient.getJsonForPost(HttpClient.getUserForLogin(mobile, pwdStr));
                    ...
                    // 注册
                    params = HttpClient.getJsonForPost(HttpClient.getUserForReg(mobile, pwdStr, nickStr, "role_1"));
                    createUser(params);
                }
            }
    ...
    private void createUser(final String params) {
        if (NetWorkUtil.isAvailable(LoginActivity.this)) {
            ...
            new Thread(new Runnable() {
                ...
                @Override
                public void run() {
                    String s = HttpClient.post(HttpClient.url + HttpClient.register, params);
                    String code = null;
                    try {
    ...
    public void httpOver(String s) {
        if (TextUtils.isEmpty(s)) {
            ...
            hideProgess();
            Toast.makeText(LoginActivity.this, "这个号码已经注册过，请重试.", Toast.LENGTH_SHORT).show();
            ...
            if (!isRegist) {
                ...
                // 登陆
                String params = HttpClient.getJsonForPost(HttpClient.getUserForLogin(et_user_account.getEditableText()
                        .toString(), et_user_psw.getEditableText().toString()));
                login(params);
            }
        } else {
            // 失败
    ...
    public void loginBackRecevie() {
        ...
        if (RemindApplication.IS_LOGIN) {
            ...
            // 是否记住用户名
            if (isRemember) {
                MySharedPreferencesLoginType.saveUserName(getApplicationContext(), et_user_account.getEditableText()
                        .toString(), et_user_psw.getEditableText().toString());
            } else {
