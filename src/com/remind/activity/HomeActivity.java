...
public class HomeActivity extends BaseActivity implements OnClickListener {
    ...
    // break;
    // // case REFRESH_START:
    // // Toast.makeText(HomeActivity.this, "start---refresh",
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        // footer = LayoutInflater.from(this).inflate(R.layout.itembottom,
        // null);
    ...
    private void initFloatingBtn() {
        ...
        // final FloatingActionButton rightLowerButton = new
    ...
    private void setUpView() {
        ...
        AbsListView.LayoutParams localObject = new AbsListView.LayoutParams(mScreenWidth, AppUtil.dip2px(this, 100));
    ...
    public void createFile() {
        ...
        // 创建音频文件夹
        File audio = new File(AppConstant.MNT + AppConstant.FILE_PATH + AppConstant.EDITED_AUDIO_PATH);
        if (!audio.exists()) {
            audio.mkdirs();
        }
        // 创建图片文件夹
        File img = new File(AppConstant.MNT + AppConstant.FILE_PATH + AppConstant.EDITED_IMG_PATH);
        if (!img.exists()) {
            img.mkdirs();
        }
        // 创建视频文件夹
        File video = new File(AppConstant.MNT + AppConstant.FILE_PATH + AppConstant.EDITED_VEDIO_PATH);
        if (!video.exists()) {
            video.mkdirs();
        }
        // 创建日志文件夹
        File log = new File(AppConstant.MNT + AppConstant.FILE_PATH + AppConstant.ERROR_PATH);
        if (!log.exists()) {
            log.mkdirs();
        }
    }
    ...
    // if ((lastItemid + 1) == totalItemCount && lastItemid < maxCount) { //
    // 达到数据的最后一条记录
    // if (totalItemCount > 0) {
    // // 当前页
    ...
    // if (notShowCount > 0) {
    // // 还有未接受提醒没有显示
    // Cursor cursor = remindDao.getUnAcceptRemind((currentpage - 1) * number,
    // notShowCount);
    // temp = DataBaseParser.getRemindDetail(cursor);
    // cursor.close();
    // // 显示今天的提醒
    // Cursor cursor1 = remindDao.getTodayRemind(today, (currentpage - 1 -
    // (notAcceptCount / 1)) * number, number - notShowCount);
    ...
    // if (loadfinish) {
    // // 显示今天提醒，今天提醒足够显示当前页
    // Cursor cursor = remindDao.getTodayRemind(today, (currentpage - 1 -
    // (notAcceptCount / 1)) * number, number);
    // temp = DataBaseParser.getRemindDetail(cursor);
    // cursor.close();
    // } else {
