...
public class AddPeopleActivity extends BaseActivity implements OnClickListener {
        ...
        public void handleMessage(android.os.Message msg) {
            ...
            case 0:
                ...
                if (null == s || !s.contains("|")) {
                    Toast.makeText(AddPeopleActivity.this, "网络连接失败，请确认后重试.", Toast.LENGTH_SHORT).show();
                }

                String[] ss = s.split("\\|");
                if (ss[0].equals("200")) {
                    // 成功
                    Toast.makeText(AddPeopleActivity.this, "发送成功，等待对方验证.", Toast.LENGTH_SHORT).show();
                    // 返回上一个界面
                    setResult(RESULT_OK);
                    finish();
                } else if (ss[0].equals("402")) {
                    // 失败, 已发送过请求或者已经是好友
                    Toast.makeText(AddPeopleActivity.this, "您已给对方发送过好友请求，或对方已是您好友。", Toast.LENGTH_SHORT).show();
                } else {
                    // 失败
                    Toast.makeText(AddPeopleActivity.this, "发送失败请重试.", Toast.LENGTH_SHORT).show();
                }
    ...
    public void onClick(View v) {
        ...
        case R.id.add_people:
            ...
            if (friend.length() <= 0) {
                Toast.makeText(AddPeopleActivity.this, "请输入用户名", Toast.LENGTH_SHORT).show();
                break;
            }

            if (peopelDao.queryPeopelByNum(friend).getCount() > 0) {
                // 已经添加联系人
        ...
        if (null == alertDialog) {
            alertDialog = new AlertDialog.Builder(this).setTitle("是否发送短息？")
                    ...
                    .setNegativeButton("取消", new DialogInterface.OnClickListener() {
                        ...
                        public void onClick(DialogInterface dialog, int which) {
                            // 发送短信
                            String phone_number = friend.trim();
...
//                                    + "邀请您加入互助提醒， 下载地址" + "XXXXXX";
                            if (phone_number.equals("")) {
                                ...
                                // }
                                // AppUtil.showToast(AddPeopleActivity.this,
                                // "发送成功");
                            }
    ...
    private void addPersonIntoDB(String friend) {
        ...
        PeopelEntity entity = new PeopelEntity("", "", friend, format.format(date), format.format(date), "",
                PeopelEntity.NORMAL, PeopelEntity.VALIDATE, "", AppConstant.USER_NUM);
