...
public class RemindEntity implements Serializable {
    ...
    /**
     * 提醒方式, 音频：{@link #AUDIO_REMIND}, 视频:{@link #VIDEO_REMIND}, 图像:
     * {@link #IMAGE_REMIND}, 默认为音频{@link #AUDIO_REMIND}
     */
    private int remindMethod = RemindEntity.AUDIO_REMIND;

    /**
     * 提醒的状态, 已接受：{@link #ACCEPT}, 未接受提醒：{@link #NEW}, 已拒绝：{@link #REFUSE},
     * 我发起的：{@link #LAUNCH}, 默认为新提醒{@link #NEW}
     */
    private int remindState = RemindEntity.NEW;

    /**
     * 我发起的提醒的状态， 等待对方接受：{@link #LAUNCH_WAIT}, 对方已拒绝：{@link #LAUNCH_REFUSE},
     * 对方已接受：{@link #LAUNCH_ACCEPT},接受后：LAUNCH_ACCEPT + 1: 关闭; 2: 已开始; 3: 马上开始;
     * 4: 延迟10分钟 默认为等待对方接受{@link #LAUNCH_WAIT}
     */
    private int launchState = RemindEntity.LAUNCH_WAIT;
    /**
     * 重复模式： 不重复：{@link #REPEAT_NO}, 每天重复：{@link #REPEAT_DAY}, 每周重复：
     * {@link #REPEAT_WEEK}, 每月重复：{@link #REPEAT_MONTH}, 每年重复：
     * {@link #REPEAT_YEAR},
     */
    private String repeatType = RemindEntity.REPEAT_NO;
    /**
     * 是否删除, {@link #NORMAL}：未删除；{@link #DELETED}：已删除 默认为未删除:{@link #NORMAL}
     */
    private String isDelete = RemindEntity.NORMAL;
    /**
     * 是否可以预览，{@link #NOT_PREV}：不能预览；{@link #CAN_PREV}：能预览 默认为不能预览:
     * {@link #NOT_PREV}
     */
     ...
     *            是否已读
     */
    public RemindEntity(String id, String ownerNum, String targetNum, String targetName, String targetNick, String addTime,
            String remindTimeMiLi, String content, String limitTime, String remindTime, String title, String repeatType,
            int isPreview, String noticeId, String ownerId, int isRead) {
        super();
        this.id = id;
     ...
     *            图片路径
     * @param remindMethod
     *            * 提醒方式, 音频：{@link #AUDIO_REMIND}, 视频:{@link #VIDEO_REMIND},
     *            图像:{@link #IMAGE_REMIND}, 默认为音频{@link #AUDIO_REMIND}
     * @param remindState
     *            提醒的状态, 已接受：{@link #ACCEPT}, 未接受提醒：{@link #NEW}, 已拒绝：
     *            {@link #REFUSE}, 我发起的：{@link #LAUNCH}, 默认为新提醒{@link #NEW}
     * @param launchState
     *            我发起的提醒的状态， 等待对方接受：{@link #LAUNCH_WAIT}, 对方已拒绝：
     *            {@link #LAUNCH_REFUSE}, 对方已接受：{@link #LAUNCH_ACCEPT},
     *            默认为等待对方接受{@link #LAUNCH_WAIT}
     * @param isDelete
     *            是否删除, {@link #NORMAL}：未删除；{@link #DELETED}：已删除 默认为未删除:
     *            {@link #NORMAL}
     * @param repeatType
     *            重复模式： 不重复：{@link #REPEAT_NO}, 每天重复：{@link #REPEAT_DAY}, 每周重复：
     *            {@link #REPEAT_WEEK}, 每月重复：{@link #REPEAT_MONTH}, 每年重复：
     ...
     *            是否已读
     */
    public RemindEntity(String id, String ownerNum, String targetNum, String targetName, String nickName, String addTime,
            String remindTimeMiLi, String title, String content, String limitTime, String remindTime, String audioPath,
            String videoPath, String imgPath, int remindMethod, int remindState, int launchState, String isDelete,
            String repeatType, int isPreview, int remindCount, String noticeId, String ownerId, int isRead) {
        super();
        this.id = id;
        this.ownerNum = ownerNum;
        this.targetNum = targetNum;
    ...
    /**
     * @return 是否可以预览，{@link #NOT_PREV}：不能预览；{@link #CAN_PREV}：能预览 默认为不能预览:
     *         {@link #NOT_PREV}
     */
