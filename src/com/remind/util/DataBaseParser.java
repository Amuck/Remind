...
public class DataBaseParser {
    ...
    public static ArrayList<PeopelEntity> getPeoPelDetail(Cursor mCursor) {
        ...
        while (mCursor.moveToNext()) {
            // String id =
            // mCursor.getString(mCursor.getColumnIndex(PeopelMsg.ID));
            ...
            PeopelEntity entity = new PeopelEntity(name, nickName, num, addTime, updateTime, imgPath, isDelete, status,
                    friendId, loginUser);
            lists.add(entity);
        }
        return lists;
    ...
    public static ArrayList<RemindEntity> getRemindDetail(Cursor mCursor) {
        ...
        while (mCursor.moveToNext()) {
            ...
            RemindEntity entity = new RemindEntity(id, ownerNum, targetNum, targetName, targetNick, addTime, lastEditTime,
                    title, content, limitTime, remindTime, audioPath, videoPath, imgPath, remindMethod, remindState,
                    launchState, isDelete, repeatType, isPreview, remindCount, noticeId, ownerId, isRead);
            lists.add(entity);
        }
        return lists;
    }
    ...
    public static ArrayList<MessageIndexEntity> getMessageIndex(Cursor mCursor) {
        ...
        while (mCursor.moveToNext()) {
            ...
            MessageIndexEntity entity = new MessageIndexEntity(id, num, message, time, name, imgPath, unReadCount, isDelete,
                    sendState, loginUser);
            lists.add(entity);
        }
        return lists;
    ...
    public static ArrayList<MessageEntity> getMessage(Cursor mCursor) {
        ...
        while (mCursor.moveToNext()) {
            ...
            MessageEntity entity = new MessageEntity(id, recieveName, recieveNum, sendName, sendNum, time, sendState,
                    isDelete, msgType, otherTypeId, msgPath, msgIndex, isComing, content, feed, loginUser, remindId);
            lists.add(entity);
        }
        return lists;
    }
