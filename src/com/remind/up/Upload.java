...
public class Upload {
    ...
    // 本地文件路径
    public static String localFilePath = Environment.getExternalStorageDirectory().getAbsolutePath() + File.separator;
    // 上传文件名
     ...
     *            过程中的回调，注意：由于在计算发送的字节数中包含了图片以外的其他信息，最终上传的大小总是大于图片实际大小，
     *            为了解决这个问题，代码会判断如果实际传送的大小大于图片，就将实际传送的大小设置成'fileSize-1000'（最小为0）
     * @param localFilePath
     *            文件绝对路径
     */
    public static void upload(Context context, String fileName, CompleteListener completeListener,
            ProgressListener progressListener, String localFilePath) {
        ...
        // savePath = "/" + filename;
        savePath = File.separator + PICTURE_PATH + File.separator + Utils.getNowDate() + File.separator
                + AppConstant.USER_NUM + "_" + filename;
     ...
     *            文件绝对路径
     */
    public static void uploadRole(Context context, String fileName, CompleteListener completeListener,
            ProgressListener progressListener, String localFilePath) {
        Upload.context = context;
        if (TextUtils.isEmpty(fileName)) {
            Toast.makeText(context, "文件无效，请重新选择", Toast.LENGTH_LONG).show();
    ...
    static class UploadTask extends AsyncTask<Void, Void, String> {
        ...
        protected String doInBackground(Void... params) {
            ...
            try {
                ...
                // @Override
                // public void transferred(long transferedBytes, long
                // totalBytes) {
                // // do something...
