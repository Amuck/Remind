...
public class RemindAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            convertView = mInflater.inflate(R.layout.item_remind_adapter, parent, false);
            viewHolder.remind_time_txt = (TextView) convertView.findViewById(R.id.remind_time_txt);
            viewHolder.remind_title_txt = (TextView) convertView.findViewById(R.id.remind_title_txt);
            viewHolder.remind_content_txt = (TextView) convertView.findViewById(R.id.remind_content_txt);
            viewHolder.remind_name_txt = (TextView) convertView.findViewById(R.id.remind_name_txt);
            viewHolder.remind_img = (RoleDetailImageView) convertView.findViewById(R.id.remind_img);
        ...
        // viewHolder.remind_name_txt.setText(remindEntity.getTargetName() +
        // remindEntity.getId());
        if ("1".equals(remindEntity.getIsDelete())) {
            // 失效的闹钟
            viewHolder.remind_img.setmBorderInsideColor(Color.DKGRAY);
        } else {
            viewHolder.remind_img.setmBorderInsideColor(remindEntity.getIsRead() == RemindEntity.NOT_READ ? Color.RED
                    : Color.WHITE);
        }
        ...
        if (0 == id) {
            // 如果头像是用户上传的图片
            if (null != imgPath && imgPath.trim().length() > 0 && viewHolder.remind_img.getTag() != null
                    && viewHolder.remind_img.getTag().equals(imgPath)) {
