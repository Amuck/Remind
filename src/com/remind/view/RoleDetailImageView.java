...
//    	init();
    ...
    public void init() {
		...
		Bitmap scaledBitmap = Bitmap.createScaledBitmap(image, width, height,
				false);
		...
		setImageBitmap(Bitmap.createBitmap(mBitmap2, 0, width, width, height,
    ...
    private boolean transformInverse(int x, int y, int[] out) {
		...
		if (distance2 > radius2) {
			...
			float amount = amplitude
					* (float) Math.sin(distance / wavelength * TWO_PI - phase
							/ TWO_PI);
		...
		for (int i = 0; i < width; i++)
			for (int j = 0; j < height; j++) {
				if (transformInverse(i, j, temp)) {
					if (temp[0] >= width || temp[1] >= height || temp[0] < 0
							|| temp[1] < 0) {
...
//			amplitude = 0.5f;
			...
			post(new Runnable() {
				...
				@Override
				public void run() {
					setImageBitmap(Bitmap.createBitmap(mBitmap2, 0, width,
